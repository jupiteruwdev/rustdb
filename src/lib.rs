...
macro_rules! table {
    ( $( $n:ident : $t:ident ),* ) => {{
        extern crate rdb;
        use std::collections::HashMap;
        let mut v: HashMap<String, rdb::Column> = HashMap::new();
        $(
            let s = stringify!($n).to_string();
            match stringify!($t) {
                "str" => v.insert(s, rdb::Column::Str),
                "block" => v.insert(s, rdb::Column::LongStr),
                "i32" => v.insert(s, rdb::Column::I32),
                "byte" => v.insert(s, rdb::Column::Byte),
                // So far just skips invalid tokens.
                _ => None
            };
        )*
        rdb::TableSpec{data:v}
    }}
}
